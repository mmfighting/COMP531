...
//     // })
// })
const salt = [Date.now(), "test"].join(':')
const newpassword = md5([salt, "123"].join(':'))
// new User({username: 'test', salt: salt, hash:newpassword }).save(function () {
//     console.log("User " + "test" + " created.")
// })
// new Profile({'username': 'test',
...
//     console.log('done with save profile for'+ 'test')
// })

new Article({ id: 1, author: 'test', img: null, date: new Date().getTime(), text: "This is Test's article", comments:[]} ).save(function() {
    console.log('done with save')
    Article.find().exec(function(err, items) {
        console.log("There are " + items.length + " articles total in db")
        findByAuthor('test', function() {
